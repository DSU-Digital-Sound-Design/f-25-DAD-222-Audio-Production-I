...
**Goal**
Create a 30–60 second audio story using only the recorded sentences in the CMU ARCTIC BDL dataset.
Your task is to locate words or short phrases within those recordings, cut them out, and arrange them in a new order using Reaper.
...
* The `wav/` folder from the CMU ARCTIC BDL dataset (hundreds of clean, full-sentence WAV files).
* **cmu-speech-recordings.zip** contains the audio plus a CSV file of all sentences for quick searching.
  
> click [here](cmu-speech-recordings.zip) to download audio files and CSV
...
2. **Import & Edit in Reaper**
   * Drag the chosen WAV files into a new Reaper session.
   * Split (`S`) to isolate the words you need.
   * Use slip editing and fades to align them naturally into sentences.
3. **Assemble Your Story**
